...
def serviceregistration(request):
	...
	if request.method == 'POST':
		...
		if service_form.is_valid():
			...
			mail = EmailMessage(
					'Welcome Message from the Team',
					"Hi "+current_user+", Thank you for signing up with brighton. We also acknowledge your request to be a service provider. Your request is for the following service(s): "+serv_reg + ", under "+serv_category+ " category. Your credentials will be vetted and response communicated to you as soon as possible. Should we require additional information, we will contact you. Thanks once again.",
					settings.EMAIL_HOST_USER,
